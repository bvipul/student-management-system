...
const validatorSemesterEditForm = (values) => {
...
class Edit extends React.Component {
    ...
    componentDidMount() {
        Server
            .get(`/api/semesters/${this.props.match.params.id}`)
            .then((response) => {
    ...
    processSubmit(values) {
        Server
            .patch(`/api/semesters/${this.props.match.params.id}`, values)
            .then((response) => {
                ...
                <CardHeader className="text-center">
                    <Row>
                        <Col md={8} className="text-right">
                            <h1 style={{ display: 'inline-block', textTransform: 'uppercase', letterSpacing: '5px' }}>Edit Semester</h1>
                        </Col>
...
Edit = reduxForm({
  form: 'semesterEdit',
  validate: validatorSemesterEditForm
